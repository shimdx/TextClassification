...
with graph.as_default():
    sess = tf.Session()
    # Restore metagraph
    saver = tf.train.import_meta_graph('{}.meta'.format(os.path.join(run_mdl2_dir, 'model',checkpoint_md2)))
    # Restore weights
    saver.restore(sess, os.path.join(run_mdl2_dir, 'model', checkpoint_md2))
    ...
    # Generate batches
    batches = data_helper.batch_iter(data_md2, labels_md2, lengths_md2, FLAGS.batch_size, 1)
    ...
    for batch in batches:
        ...
        if 'cnn' in params['clf']:
            ...
            batch_size = graph.get_tensor_by_name('batch_size:0')
            sequence_length = graph.get_tensor_by_name('sequence_length:0')
            feed_dict = {input_x: x_test, input_y: y_test, batch_size: min(FLAGS.batch_size, len(y_test)), sequence_length: x_lengths, keep_prob: 1.0}
...
cm_md2 = pd.DataFrame(confusion_matrix(labels_md2, all_predictions), columns=["dialog", "life", "bot", "shp", "tour"], index=["dialog", "life", "bot", "shp", "tour"])
print(cm_md2)
